...
# 0. Limpa containers anteriores (se existirem)
echo " Limpando containers anteriores..."
docker stop test-mongo test-python 2>/dev/null || true
docker rm test-mongo test-python 2>/dev/null || true
echo ""
...
docker exec -i test-mongo mongoimport \
  --db transcendence \
  --collection profiles \
  --jsonArray < tests/test-data.json
